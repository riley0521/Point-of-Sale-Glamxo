...
namespace POSWinforms.Transaction
{
    public partial class frmSelectItem : Form
    {
        ...
        private readonly long invoiceID = 0;
        private tblItem item;

        public frmSelectItem(long invoiceID)
        {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            if (item != null)
            {
                txtTotal.Text = getGrandTotal(item.UnitPrice, quantity, discount).ToString("0.00");
            }
        ...
        private void addToCart()
        {
            if (item != null)
            {
                ...
                if (isItemExisting != null)
                {
                    ...
                    isItemExisting.Total = frmSelectItem.getGrandTotal(item.UnitPrice, isItemExisting.Quantity, isItemExisting.Discount);
                }
                else
                {
